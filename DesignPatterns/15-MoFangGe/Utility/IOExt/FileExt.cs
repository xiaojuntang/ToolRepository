...
namespace Climb.Utility.IOExt
{
    ...
    public class FileExt
    {
        ...
        public static string ReadFileFromEmbedded(string fileWholeName)
        {
            ...
            using (TextReader reader = new StreamReader(stream: Assembly.GetExecutingAssembly().GetManifestResourceStream(fileWholeName)))
            {
        ...
        #region 格式化路径
        /// <summary>格式化路径</summary>
        /// <param name="sFilePath">路径字符串</param>
        /// <returns>路径字符串</returns>
        // ReSharper disable once MemberCanBePrivate.Global
        public static string FormatPath(string sFilePath)
        {
            string sNewPath = sFilePath.Replace("\\", "/");
            if (!sNewPath.EndsWith("/")) sNewPath += "/";
            return sNewPath;
        }

        /// <summary>反格式化路径</summary>
        /// <param name="sFilePath">路径字符串</param>
        /// <returns>路径字符串</returns>
        public static string FormatPathTo(string sFilePath)
        {
            string sNewPath = sFilePath.Replace("/", "\\");
            if (!sNewPath.EndsWith(@"\")) sNewPath += @"\";
            return sNewPath;
        ...
        #region 发布程序的根目录的物理路径
        /// <summary>发布程序的根目录的物理路径</summary>
        public static string  RootPhysics
        {
            get
            {
                //String sPath = HttpContext.Current.Server.MapPath("~/");
                string sPath = @HostingEnvironment.ApplicationPhysicalPath;
                if (sPath.Length == 0) sPath = HttpContext.Current.Server.MapPath("~/");
                if (sPath.Length > 0) sPath = FormatPath(sPath);
                return sPath;
            }
        }
        #endregion

        #region 发布程序的根目录的虚拟路径
        /// <summary>发布程序的根目录的虚拟路径</summary>
        public static string RootVirtual
        {
            get
            {
                string sPath = @HostingEnvironment.ApplicationVirtualPath;
                if (sPath.Length == 0) sPath = HttpContext.Current.Request.ApplicationPath;
                if (sPath.Length > 0) sPath = FormatPath(sPath);
                return sPath;
            }
        }
